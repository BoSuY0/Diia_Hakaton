...
from src.sessions.models import Session, SessionState, FieldState
...
def test_update_session_field():
    ...
    from src.categories.index import list_party_fields
    ...
    ok, error, fs = update_session_field(session, field_name, "Test Value", role="lessor")
    ...
    if iban_field:
        ...
        ok, error, fs = update_session_field(session, iban_field, "INVALID_IBAN", role="lessor")
        assert ok is False
    ...
    history = session.all_data[f"lessor.{field_name}"]["history"]
    assert len(history) >= 2 # +1 from first test
    assert history[-1]["value"] == "Value 2"
    ...
    # If company not in allowed types, this might fail validation in real app, but here we test service logic
    ...
    if session.state == SessionState.TEMPLATE_SELECTED: # or READY_TO_BUILD if ready
