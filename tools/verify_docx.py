...
def test_regex_cleanup():
    ...
    for input_text, expected in test_cases:
        ...
        for label in labels_to_clean:
            ...
            # Let's verify if my refactor matches the original behavior for the test cases.
            text = re.sub(rf"{safe_label}:\s*$", "", text)
        ...
        # Adjust expectation for "Mixed" case because "РНОКПП: , " matches "Label followed by comma" pattern?
        # "РНОКПП: , end" -> "РНОКПП: ," matches "РНОКПП:\s*,\s*" ?
