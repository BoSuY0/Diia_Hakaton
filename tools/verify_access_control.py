...
def test_access_control():
    ...
    resp = client.post("/sessions", json={}, headers={"X-User-ID": creator_id})
    assert resp.status_code == 200
    session_id = resp.json()["session_id"]
    print(f"Session created: {session_id}")

    # Ensure no roles are assigned automatically
    summary = client.get(f"/sessions/{session_id}", headers={"X-User-ID": creator_id}).json()
    assert summary.get("role_owners") == {} or summary.get("role_owners") is None
    ...
    if resp.status_code != 200:
        ...
        return
    assert resp.status_code == 200
    # role_owners should reflect claim
    summary = client.get(f"/sessions/{session_id}", headers={"X-User-ID": user1_id}).json()
    assert summary["role_owners"].get(role1) == user1_id
    ...
    resp = client.get(f"/sessions/{session_id}/schema", headers={"X-User-ID": user3_id})
    if resp.status_code == 403:
    ...
    resp = client.get(f"/sessions/{session_id}", headers={"X-User-ID": user1_id})
    if resp.status_code == 200:
