...
async def test_lifespan_skips_cleanup_for_non_fs(monkeypatch, mock_settings):
    ...
    async def fake_clean_abandoned(
        _active_ids, _grace_period_minutes=5
    ):
    ...
    monkeypatch.setattr(
        "backend.domain.sessions.cleaner.clean_abandoned_sessions",
        fake_clean_abandoned,
    )
    monkeypatch.setattr(
        "backend.domain.sessions.cleaner.clean_stale_sessions", fake_clean_stale
    )
    monkeypatch.setattr(
        "backend.api.http.server.stream_manager.shutdown", fake_shutdown
    )
