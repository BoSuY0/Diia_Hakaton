...
export const ContractDetails = ({ sessionId, userId, onBack, onEdit }) => {
    ...
    const renderHistory = () => {
        ...
        if (!history || !history.history || history.history.length === 0) {
            return <div className="info-text">Історія поки відсутня.</div>;
        }

        const events = [...history.history].sort(
            (a, b) => new Date(b.ts || b.timestamp || b.time) - new Date(a.ts || a.timestamp || a.time)
        );
        const signEvents = events.filter(e => e.type === 'sign');
        const fieldEvents = events.filter(e => e.type === 'field_update').slice(0, 20);
            ...
            <div className="history-grid">
                <div className="history-card">
                    ...
                    {signEvents.length === 0 ? (
                        ...
                        <div key={`${evt.ts || evt.timestamp || idx}`} className="history-row">
                            <div className="history-meta">
                                <span className="history-pill">
                                    {(evt.roles || []).map((r) => labelForRole(r)).join(', ') || 'роль'}
                                </span>
                                <span className="history-timestamp">{formatTimestamp(evt.ts || evt.timestamp)}</span>
                            </div>
                            <div className="history-detail">
                                Користувач: {evt.user_id || 'N/A'} • Стан: {evt.state}
                            </div>
                        </div>
                    ))}
                </div>
