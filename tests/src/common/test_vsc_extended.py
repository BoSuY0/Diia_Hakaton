...
def test_vsc_entities_formats_required_flag():
    """Test VSC entities formats required flag."""
    res = vsc.vsc_entities({
        "entities": [{"field": "f1", "label": "L", "type": "text", "required": True}]
    })
...
def test_vsc_upsert_result_can_build_flag():
    """Test VSC upsert result can build flag."""
    out = vsc.vsc_upsert_result({
        "field": "f", "status": "ok", "error": None,
        "state": "built", "can_build_contract": True
    })
...
def test_vsc_summary_contains_fields():
    """Test VSC summary contains fields."""
    out = vsc.vsc_summary({
        "state": "s1",
