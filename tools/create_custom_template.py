"""Script to create custom DOCX contract template."""
import os
...
def create_custom_template() -> None:
    """Create a custom contract template with placeholders."""
    doc = Document()
    ...
    # Parties
    doc.add_paragraph(
        "Сторона 1: {{ party_a.name }}, що діє на підставі власного волевиявлення, "
        "надалі іменується «Сторона 1», та"
    )
    doc.add_paragraph(
        "Сторона 2: {{ party_b.name }}, що діє на підставі власного волевиявлення, "
        "надалі іменується «Сторона 2»,"
    )
