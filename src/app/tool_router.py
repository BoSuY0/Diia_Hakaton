...
async def tool_sign_contract_async(session_id: str, role: Optional[str] = None) -> Dict[str, Any]:
    tool = tool_registry.get("sign_contract")
    if tool:
        return await tool.execute_async({"session_id": session_id, "role": role}, {})
    return {}


# Backward-compatible aliases (unused; kept for imports)
tool_find_category_by_query = tool_find_category_by_query
tool_get_templates_for_category = tool_get_templates_for_category
tool_get_category_entities = tool_get_category_entities
tool_get_category_parties = tool_get_category_parties
tool_get_party_fields_for_session = tool_get_party_fields_for_session
tool_set_category = tool_set_category
tool_set_template = tool_set_template
tool_upsert_field = tool_upsert_field
tool_get_session_summary = tool_get_session_summary
tool_build_contract = tool_build_contract
tool_set_party_context = tool_set_party_context
tool_sign_contract = tool_sign_contract
